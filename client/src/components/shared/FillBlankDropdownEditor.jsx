...
const FillBlankDropdownEditor = ({ value, onChange }) => {
    ...
    const removeBlankOption = (segmentIndex, optionIndex) => {
        ...
        if (newSegments[segmentIndex].options.length <= 2) {
            setError(t.blankMinOptionsError || 'A blank must have at least 2 options.');
            return;
        }
    ...
    const renderPreview = () => {
        if (!segments || segments.length === 0) return <span className="text-gray-400 italic">{t.noContentPassagePreview || 'No content yet. Add text and blanks to build your passage.'}</span>;
            ...
            if (seg.type === 'text') {
                ...
                return (
                    <select key={seg.id} className="mx-1 px-2 py-1 bg-white border border-gray-300 rounded text-sm inline-block max-w-xs align-middle truncate">
                        <option value="">{t.chooseBlankOption || '— choose —'}</option>
                        {seg.options.map((opt, i) => (
                            <option key={i} value={opt}>{opt || (t.emptyOptionLabel || '(empty option)')}</option>
                        ))}
                    </select>
                );
            }
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <label className="block text-sm font-medium text-gray-700">
                        {t.passageStructure || 'Passage Structure'}
                    </label>
                    <span className="text-xs text-gray-500">
                        {t.passageStructureSubtitle || 'Build your passage by mixing Text and Blank blocks.'}
                    </span>
                </div>
                    ...
                    {segments.map((segment, index) => (
                        <div key={segment.id} className={`flex gap-3 items-start p-3 border rounded-lg bg-white shadow-sm transition-all ${segment.type === 'blank' ? 'border-indigo-200' : 'border-gray-200'}`}>
                            ...
                            <div className="flex-1 space-y-2">
                                <div className="flex items-center gap-2 mb-2">
                                    <span className={`text-xs font-semibold px-2 py-1 rounded uppercase tracking-wider ${segment.type === 'text' ? 'bg-gray-100 text-gray-600' : 'bg-indigo-100 text-indigo-700'}`}>
                                        {segment.type === 'text' ? (t.textBlockBadge || 'Text Block') : (t.dropdownBlankBadge || 'Dropdown Blank')}
                                    </span>
                                </div>
                                ...
                                {segment.type === 'text' ? (
                                    <textarea
                                        ...
                                        placeholder={t.typePassagePlaceholder || "Type passage text here..."}
                                    />
                                ) : (
                                    <div className="space-y-3">
                                            ...
                                            <div key={optIndex} className="flex gap-2 items-start">
                                                <div className="pt-2">
                                                    <input
                                                        ...
                                                        title={t.markAsCorrectAnswer || "Mark as correct answer"}
                                                    />
                                                </div>
                                                <div className="flex-1">
                                                    <textarea
                                                        ...
                                                        placeholder={`${t.optionNumberPlaceholder || 'Option'} ${optIndex + 1}`}
                                                ...
                                                <button
                                                    ...
                                                    title={t.removeOption || "Remove option"}
                                                >
                                        ...
                                        >
                                            <Plus className="w-4 h-4" /> {t.addOption || 'Add Option'}
                                        </button>
                                    </div>
                                )}
                            </div>
                            ...
                            <button
                                ...
                                title={t.removeEntireBlock || "Remove entire block"}
                            >
                ...
                <div className="flex gap-3 pt-2 border-t border-gray-100 mt-4">
                    ...
                    >
                        <Type className="w-4 h-4" /> {t.addText || 'Add Text'}
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        <List className="w-4 h-4" /> {t.addDropdownBlank || 'Add Dropdown Blank'}
                    </button>
                </div>
            </div>

            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mt-6">
                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t.livePreview || 'Live Preview'}</h4>
