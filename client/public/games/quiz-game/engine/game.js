  ...
  setTimeout(() => {
    ...
    if (gameState.currentIndex < gameState.questions.length) {
      ...
      WajibetSDK.recordInteraction({
        itemId: `q_${gameState.currentIndex}`,
        itemIndex: gameState.currentIndex,
        type: 'multiple_choice',
        isCorrect: isCorrect,
        userAnswer: selectedBtn.textContent,
        correctAnswer: currentQ.correctAnswer,
        score: gameState.score,
        maxScore: parseInt(gameState.settings.pointsPerQuestion) || 10,
        timeMs: elapsedMs,
        attempts: 1,
        skipped: false
      });
...
function endGame() {
  ...
  WajibetSDK.finishGame(gameState.score, elapsedMs);
}
