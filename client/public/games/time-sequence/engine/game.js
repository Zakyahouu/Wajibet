(function () {
    ...
    WG.run({
        ...
        reveal: function (q) {
            ...
            nodes.forEach(function (node, slotIndex) {
                var card = node.querySelector('.event-card');
                if (card) {
                    card.classList.add('is-locked');
                    // FIX: find which event this card belongs to, check its correctIndex
                    var cardEventIndex = q.events.findIndex(function (ev) { return ev.id === card.id; });
                    var isCorrectSlot = cardEventIndex !== -1 && q.events[cardEventIndex].correctIndex === slotIndex;
                    ...
                    if (isCorrectSlot) {
                        ...
                        // Show what the correct event for this slot was
                        var correctEv = q.events.find(function (ev) { return ev.correctIndex === slotIndex; });
                        var correctLabel = correctEv ? correctEv.text : '';
                        node.insertAdjacentHTML('beforeend',
                            '<span class="tag">Wrong Spot</span>' +
                            (correctLabel ? '<span class="tag tag-hint">Should be: ' + correctLabel + '</span>' : ''));
                    }
                } else {
                    node.classList.add('is-wrong');
                }
